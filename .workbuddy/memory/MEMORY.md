...
- AI 解析使用 DeepSeek `deepseek-v4-flash` 模型（$0.14/M 输入 + $0.28/M 输出），批量解析 max_tokens=16000，单题分析 max_tokens=2000
- DeepSeek API Key 通过云函数环境变量 `DEEPSEEK_API_KEY` 配置，不在代码中硬编码
- 云函数 `aiParse` 已配置 `config.json` 设置 60s 超时（默认 20s 会导致 DeepSeek API 调用被 kill）
- `normalizeQuestion` 函数返回值必须用已标准化的 `content` 变量，不能用 `q.content`（会导致非 content 字段名的题目内容变成 "undefined"）
- AI 解析前端有 30s 超时保护（Promise.race），微信错误对象用 `errMsg` 而非 `message` 属性
